...
import DefaultOptions from "./option/defaultOptions";
import { DisplaySize } from "./option/displaySize";
import { Resize } from "./option/resize";
import defaultOptions from "./option/defaultOptions";
const BlockEmbed = Quill.import("blots/block/embed");
// import { Toolbar } from "./option/toolbar";
...
export class VideoResize {
  constructor(quill, options = {}) {
    ...
    let moduleClasses = defaultOptions.modules;
  ...
  handleClick(evt) {
    if (evt.target === this.quill.root) {
      ...
      for (let i = 0; i < videos.length; ++i) {
        ...
        if (evt.clientX < rect.x - 10) {
          continue;
        }
        if (evt.clientX > rect.right + 10) {
          continue;
        }
        ...
        if (
          videos[i] &&
          videos[i].tagName &&
          videos[i].tagName.toLowerCase() === this.options.tagName
        ) {
